...
#video_path = project_folder + '/space background.mp4'
filename = ""
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.configure(fg_color="#FFDAB9")#background
        self.title("Bone Fracture Detection")
        self.geometry(f"{500}x{740}")
        #self.head_frame = ctk.CTkFrame(master=self)
        self.head_frame = ctk.CTkFrame(master=self, fg_color="#2F4F4F")  # Teal background
        self.head_frame.pack(pady=20, padx=60, fill="both", expand=True)
        self.main_frame = ctk.CTkFrame(master=self, fg_color="#2F4F4F")
        self.main_frame.pack(pady=20, padx=60, fill="both", expand=True)
        self.head_label = ctk.CTkLabel(master=self.head_frame, text="Bone Fracture Detection System",
                                       font=(ctk.CTkFont("Roboto"), 35))
        self.head_label.place(relx=0.5, rely=0.5, anchor="center")
        ...
        self.info_label = ctk.CTkLabel(master=self.main_frame,
                                       ...
                                       wraplength=300, font=(ctk.CTkFont("Roboto"), 25))
        self.info_label.pack(pady=10, padx=10)

        self.upload_btn = ctk.CTkButton(master=self.main_frame, text="Upload Image", command=self.upload_image,
                                        width=150, height=50)  # Adjust the values as needed
        self.upload_btn.pack(pady=0, padx=1)
##############
        self.frame2 = ctk.CTkFrame(master=self.main_frame, fg_color="#FFDAB9", width=256, height=256)
        ...
        self.predict_btn = ctk.CTkButton(master=self.main_frame, text="Predict", command=self.predict_gui,
                                         width=150, height=50)  # Adjust the width and height values as needed
